...
(function ($) {
    ...
    $(window).on('load', function (evt) {
        $('input.vb-prop-set').each(function (i, el) {
            ...
            var values = $.parseJSON($(el).val());
            var id = $(el).attr('id');
            var item = $('#' + id + '-list').find('*[data-set="' + key + '"]');
            var keyIn = $('#' + id + '-key');
            keyIn.on('change select click', function (eventObject) {
            ...
            $('#' + id + '-list').find('.icon-cancel').on('click', function (eventObject) {
                var itemKey = $(eventObject.target).parents('li').attr('data-set');
                values[itemKey] = undefined;
                setVal($(el), JSON.stringify(values));
                window['Joomla'].submitbutton('plugin.apply');
            });
            form.find('input, select, textarea').on('change select click', function (eventObject) {
                var name = $(eventObject.target).attr('name');
                if (name) {

...
(function($: JQueryStatic) {
    ...
    $(window).on('load', function(evt: JQueryEventObject) {
        ...
        $('input.vb-prop-set').each(function(i: number, el: Element) {
            ...
            var values: any = $.parseJSON($(el).val());
            var id: string = $(el).attr('id');
            var item: JQuery = $('#' + id + '-list').find('*[data-set="' + key + '"]');

            var keyIn: JQuery = $('#' + id + '-key');
            keyIn.on('change select click', function(eventObject: JQueryEventObject) {
                var name = $(eventObject.target).attr('name');
                if (name) {
                    var value = getVal(form, name).replace(/(^\W+)|(\W+$)/g, '').replace(/\W+/g, '-').toLowerCase();
                    if (!value || value == 'new') return false;
                    setVal($(eventObject.target), value);
                    if (key != value) {
            ...
            $('#' + id + '-list').find('.icon-cancel').on('click', function(eventObject: JQueryEventObject) {
                var itemKey = $(eventObject.target).parents('li').attr('data-set');
                values[itemKey] = undefined;
                setVal($(el), JSON.stringify(values));
                window['Joomla'].submitbutton('plugin.apply');
            });

            form.find('input, select, textarea').on('change select click', function(eventObject: JQueryEventObject) {
                var name = $(eventObject.target).attr('name');
                if (name) {
